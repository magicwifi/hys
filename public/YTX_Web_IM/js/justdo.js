...
(function() {
    window.IM = window.IM || {
        ...
        _3rdServer : 'http://123.57.230.158:8886/authen/genSig', // 3rdServer，主要用来虚拟用户服务器获取SIG
        ...
        init : function() {
            ...
            // 初始化SDK
            var resp = RL_YTX.init(IM._appid,null,null);
            if (!resp) {
                alert('SDK初始化错误');
                return;
            }
        ...
        EV_keyCode : function(event) {
            ...
            }else if(16 == IM._keyCode_1 && 50 == IM._keyCode_2){
                //判断如果是群组的话才展示成员列表
            ...
            }else if(16 == IM._keyCode_1 && 229 == IM._keyCode_2){//中文输入法时返回229
                setTimeout(function(){
        ...
        DO_logout : function() {
            ...
            if(IM.localMediaStream!=null){
                 IM.localMediaStream.stop();
                 IM.localMediaStream = null;
            }
        ...
        EV_sendTextMsg : function(oldMsgid, text, receiver,isresend) {
            ...
            var msgId = RL_YTX.sendMsg(obj, function(obj) {
                        setTimeout(function() {
                                    ...
                                    if(isresend){
                                        ...
                                        msg.remove();// 删掉原来的展示
                                        $(document.getElementById(receiver + "_" + obj.msgClientNo)).find('span[imtype="resend"]').click(function() {
                                            IM.EV_resendMsg(receiver, obj.msgClientNo);
                                        });
                                    };
                                }, 300)
                    }, function(obj) {
            ...
            oldMsg = document.getElementById(receiver + '_' + msgId);
            $(oldMsg).find('span[imtype="resend"]').removeAttr("onclick");
            $(oldMsg).find('span[imtype="resend"]').click(function() {
                        IM.EV_resendMsg(receiver, msgId);
                    });
        },
         ...
         * @constructor
         */
        EV_resendMsg : function(receiver, oldMsgid) {
            var msg = $(document.getElementById(receiver+"_"+oldMsgid));
        ...
        /*  $('#im_content_list').append(msg.prop("outerHTML"));
            ...
            if (1 == msgtype) {// 文本消息
                ...
                if (IM._contact_type_m == contact_type) {
                    IM.EV_sendMcmMsg(msgid, text, content_you,true);
                }else{
                ...
                if (!!oFile) {
                    ...
                    var dataurl = msg.find('input[imtype="msg_attach_blobImg"]').val();
                    ...
                    oFile = new Blob([buffer.buffer], {type: 'image/png'});
        ...
        EV_sendAttachMsg : function(oldMsgid, file, type, receiver,isresend) {
            ...
            if (!!oldMsg.find('img[imtype="msg_attach_src"]')) {

                oldMsg.find('img[imtype="msg_attach_src"]').click(function() {
                            IM.DO_pop_phone(receiver, msgid);
                        });
            }
        },
        ...
        HTML_pushMsg_addHTML : function(msgtype, you_sender, version,
            ...
            if(you_msgContent.indexOf("fireMsg") > -1){//fireMsg="yes"
                ...
                var fireMsgStr = '<img style="max-width:'+imgWid+'px; max-height:'+imgHei+'px; margin-right:5px; margin-left:5px;" ' +
                        'src="assets/img/fireMessageImg.png" onclick="IM.DO_showFireMsg(\''+ id +'\')" />';
        ...
        HTML_sendMsg_addHTML : function(msg, msgtype, msgid, content_type,
            ...
            var str = '<div contactor="sender" im_carousel="'+carou+'" msg="'
                    ...
                    + display
                    + '">'
                    + '<span imtype="resend" class="add-on" onclick="IM.EV_resendMsg('
                    + content_you
                    + ','
                    + msgid
                    ...
                    /*
                     * + '<a class="pull-right" href="#" style="width:150px;height:20px;word-break:keep-all;text-overflow:ellipsis;overflow: hidden;text-decoration:none">&nbsp;:' + IM._username
                    + '</a>' + im_send_content + '</div>';
                     */
        ...
        EV_getUserState : function(contact_you){
            ...
            RL_YTX.getUserState(getUserStateBuilder,function(obj){
                if(obj.state == 1){//1在线 2离线
                    ...
                    alert("非法的返回值obj.state = "+obj.state)
                }
            },function(obj){
                if(174006 != obj.code){
                	alert(obj.code);
        ...
        DO_html_create : function(groupName,permission) {
            ...
            if(permission == 4){
                ...
                str += '<tr>'
                    ...
                    + '<div style="height:auto;padding-top:5px;width:30%;float:left">'
                    ...
                    + '</select>'
                    + '</div>'
                    + '<div style="height:auto;padding-top:5px;width:30%;float:left">'//如果屏幕过小的时候就分成两列 
            ...
            for (var i in obj) {
                ...
                if (member.role == 1 || member.role == 2) {
                    ...
                    str += '<tr contact_you="'
                            ...
                            + '<a href="#" onclick="IM.HTML_editGroupMemNick(this)" '
                            + ' style="max-width:70%;word-break:keep-all;text-overflow:ellipsis;overflow: hidden;text-decoration:none">'
                            + member.nickName + '</a><input style="display:none;padding:0px;margin-bottom:0px" type="text" onblur="IM.checkNick(this); IM.modifyName()" /></span></td>' + '</tr>';
                    ...
                    str += '<tr contact_you="'
                            ...
                            + member.nickName + '</a><input style="display:none;padding:0px;margin-bottom:0px" type="text" onblur="IM.checkNick(this); IM.modifyName()" /></span>';
                    if (isowner) {
        ...
        HTML_popAddMember : function(groupId, memberId, memberName, isowner,
            ...
            var str = '<tr contact_you="'
                    ...
                    + '<a href="#"  onclick="IM.HTML_editGroupMemNick(this)" ' 
                    + 'style="max-width:50%;word-break:keep-all;text-overflow:ellipsis;overflow: hidden;text-decoration:none">'
                    + memberName + '</a><input style="display:none;padding:0px;margin-bottom:0px" type="text" onblur="IM.checkNick(this); IM.modifyName()" /></span>';
            if (isowner && isowner == 'true') {
                str += '<span class="pull-right label label-warning" onclick="IM.EV_deleteGroupMember(\''
        ...
        HTML_editGroupMemNick : function(obj){
             
            IM.editingMember(obj);
            
            if($('#pop').find("h3").text().indexOf("讨论组") > -1){//如果是讨论组不允许修改
                return; 
            }
            IM.DO_rightMenuNick(); 
            
            //if(e.button == "2"){ //0 左键 1 中间 2 右键
            //    var left = e.clientX; //鼠标在当前窗口的横坐标
            //    var top = e.clientY + $(window).scrollTop() ; //鼠标在当前窗口的纵坐标和卷起的高度
            //    $("#editNickMenu").css("left",left+"px");
            //    $("#editNickMenu").css("top",(top-80)+"px");
            //    $("#editNickMenu").css("visibility","visible"); //用visibility而不是display，是因为visibility不会因为显示或隐藏导致外面高度变化，因为该属性不会在隐藏的时候释放占用空间。
            //    //禁止滚动条的拖动
            //    $("#editNickMenu").next().find('div[class="modal-body"]').scroll(function(){
            //        $ (this).scrollTop (0);
            //    });
            //   
            //    $(window).click(function(){
            //        $("#editNickMenu").css("visibility","hidden");
            //        $("#editNickMenu").next().find('div[class="modal-body"]').unbind ('scroll'); //允许滚动条拖动
            //    });
            //};
        ...
        editingMember : function(obj){
            var memberId = $(obj).parent().parent().parent().attr("contact_you");
            $("#editNickMenu").attr("memberId",memberId);
             
            var memberName = $(obj).html();
            $("#editNickMenu").attr("memberName",memberName);
        },
        DO_rightMenuNick : function(){
            var memberId = $("#editNickMenu").attr("memberId");
            var currentTr = $("#pop").find('tr[contact_you=\"'+memberId+'\"]');
            currentTr.find("a").hide();
             
            var memberName = $("#editNickMenu").attr("memberName");
            currentTr.find("input").val(memberName);

            currentTr.find("input").show();
            currentTr.find("input").focus();
        },

        modifyName : function(){
            var memberId = $("#editNickMenu").attr("memberId");
            var nick = $("#pop").find('tr[contact_you=\"'+memberId+'\"]').find("a").text();
        ...
        insertText : function (obj,str,startIndex) {
     
            var startPos = parseInt(startIndex)+1;
	        var endPos = startPos; 
	        var cursorPos = startPos;
	        var tmpStr = obj.innerText;
	        obj.innerText = tmpStr.substring(0, startPos) + str + tmpStr.substring(endPos, tmpStr.length);
        ...
        DO_im_attachment_file_up : function(id, oldMsgid) {
            ...
            var current_contact_type = IM.HTML_find_contact_type();
            if (IM._contact_type_m == current_contact_type) {
                IM.EV_sendToDeskAttachMsg(oldMsgid, oFile, 6, receiver);
            } else {
                IM.EV_sendAttachMsg(oldMsgid, oFile, 6, receiver);
            }
        ...
        DO_takePicture : function() {
            ...
            if (!!(navigator.getUserMedia)) {
                ...
                window.URL = window.URL || window.webkitURL || window.mozURL || window.msURL;
                // 调用遮罩层定位摄像头
                var str = '<div id="takePhoto" class="modal" style="position: relative; top: auto; left: auto; right: auto; margin: 0 auto 0px; z-index: 1; max-width: 100%;text-align:center;border:0px;display:block">'
                        + '<video autoplay id="video" width="100%" style="position:relative;top:-20px;left:0px" ></video>'
                        + '<div style="position:relative;bottom:25px;left:auto;right:auto;padding-top:5px;background:#333">'
                        + '<a onclick="IM._picClose();" style="position:relative;right:10%;text-decoration: none;color:white">取消</a>'
                        + '<img class="img-circle" src="assets/img/white.png" id="snap" />'
                        + '</div></div>';

                $('#pop_takePicture').find('div[imtype="pop_takePicture_top"]').html(str);
                // 拍照按钮的浏览器兼容
                var windowWid = $(window).width();
                if (windowWid < 666) {
                ...
                var onFailSoHard = function() {
                    alert("设备拒绝访问")
                    IM.HTML_pop_takePicture_hide();
//                  IM.DO_im_image_file();// 如果不支持摄像头的通过本地文件夹获取
                };
                ...
                navigator.getUserMedia({
                            ...
                            IM.localMediaStream = stream;
                            $("#takePhoto").css("height",$("#takePhoto").css("height").split("px")[0]- 50);

                            $("#snap").click(function() {
                                IM._snapshot(content_type, content_you);
                            });
                            setTimeout(function() {
         ...
         * 拍照
         */
        _snapshot : function(content_type, content_you) {
            if (IM.localMediaStream) {
                ...
                if (windowWid < 666) {
                    ...
                    var canvasWidthInt = parseInt(canvas.width);
                    canvas.height = (canvasWidthInt / 6) * 5;
                };
                ...
                var str1 = '<div class="progress progress-striped active">'
                        ...
                        + '</div>'
                        + '<span imtype="msg_attach">'
                        + '<img imtype="msg_attach_src" src="#" style="max-width:'+ imgWid + 'px;max-height:' + imgHei + 'px;" />'
                        + '<input imtype="msg_attach_blobImg" style="display:none;" value="'+dataurl+'"></input>'
                        + '</span>';

                var id = IM.HTML_sendMsg_addHTML('msg', 4, msgid, content_type,content_you, str1);
                
                $(document.getElementById(id)).find('img[imtype="msg_attach_src"]').attr('src',dataurl);
            /*    window.URL = window.URL || window.webkitURL || window.mozURL || window.msURL;
                var url = window.URL.createObjectURL(oFile);
                $(document.getElementById(id)).find('img[imtype="msg_attach_src"]').attr('src', url);*/
                //抽取DataURL中的数据部分，从Base64格式转换为二进制格式
                var bin = atob(dataurl.split(',')[1]);
                //创建空的Uint8Array
                var buffer = new Uint8Array(bin.length);
                ...
                blob.fileName=".jpg";
                IM.DO_im_image_file_up(id, msgid, blob);
            };
            video.src = "";
            IM.localMediaStream.stop();
            IM.HTML_pop_takePicture_hide();
       ...
       DO_startRecorder : function(){
            ...
            context.fillRect(0,0,windowWidth,windowHeight+200);
            $("#pop_recorder").show();
            $("#pop_recorder").find("div").css("top",windowHeight/2);
            var width = $("#pop_recorder").find("div").width();
            $("#pop_recorder").find("div").css("left",(windowWidth - width)/2+"px");
            HZRecorder.get(function (rec) {
            	IM._recorder = rec;
            	IM._recorder.start($("#recorderAudio"));
                
            });
       },
       voiceBlob:null,
       DO_endRecorder : function(){
    	   IM.voiceBlob = IM._recorder.getBlob();
           window.URL = window.URL || window.webkitURL || window.mozURL || window.msURL;
           var url = window.URL.createObjectURL(IM.voiceBlob);
           IM._sendRecorder(url);
       },
       _sendRecorder : function (url) {
            // 初始化右侧对话框消息
            var str1 = '<div class="progress progress-striped active">'
                    + '<div class="bar" style="width: 20%;"></div>'
                    + '</div>'
                    ...
                    + '<input imtype="msg_attach_blobVoice" style="display:none;" value="'+url+'"></input>'
                    + '</span>';
	        var msgid = new Date().getTime();
            var content_type = '';
            var content_you = '';
            ...
            $("#pop_recorder").hide();
            var current_contact_type = IM.HTML_find_contact_type();
            IM.voiceBlob.fileName = ".wav";
            if (IM._contact_type_m == current_contact_type) {
                IM.EV_sendToDeskAttachMsg(msgid, IM.voiceBlob, 2, content_you);
            } else {
                IM.EV_sendAttachMsg(msgid, IM.voiceBlob, 2, content_you);
            };
